...
import org.smart.framework.dao.SqlHelper;
...
public class DataSet {
     ...
     * 插入一条数据
     */
    public static Serializable insertReturnPK(Class<?> entityClass, Map<String, Object> fieldMap) {
        if (MapUtil.isEmpty(fieldMap)) {
            return true;
        }
        String sql = SqlHelper.generateInsertSql(entityClass, fieldMap.keySet());
     ...
     * 插入一个实体
     */
    public static Serializable insertReturnPK(Object entity) {
        if (entity == null) {
            throw new IllegalArgumentException();
        }
        Class<?> entityClass = entity.getClass();
